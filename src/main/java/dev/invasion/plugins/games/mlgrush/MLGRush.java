...
public final class MLGRush extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        setPlayerDataManager(new PlayerDataManager());
        //hardcode Map
        save();
        load();
    ...
    public void load() {
        ...
        try {
            ...
            e.printStackTrace();
            this.getLogger().info("error couldn't load File");
        }
    }
    public void save() {
        mapManager.addMap(new gameMap(new SerializableLocation(1, 1,1), new BoundingBox(40,0,40, -40, 40,-40 )));
        try {
            FileManager.save(mapManager, "Maps.json");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
